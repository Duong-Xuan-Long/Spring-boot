//{id}/forgot-password
const URL_API = "http://localhost:8081/api/v1/users";
...
const show = async () => {
    try {
        let res = await axios.get(`${URL_API}/${idURL}`);
...
btnModalImage.addEventListener("click", async () => {
    try {
        let res = await axios.get(`${URL_API}/${idURL}/files`);
...
avatar.addEventListener("change", async (e) => {
    try {
        ...
        //tao api
        let res = await axios.post(`${URL_API}/${idURL}/files`, formData);
...
btnDeleteImage.addEventListener("click", async () => {
    ...
    try {
        await axios.delete(`${URL_API}/${idURL}/files/${fileId}`);
...
forgotButton.addEventListener("click", async () => {
    try {
        await axios.get(`${URL_API}/${idURL}/forgot-password`);
    } catch (error) {
        console.log(error);
    }
})
...
confirmButton.addEventListener("click", async () => {
    try {
        await axios.put(`${URL_API}/${idURL}/update-password`, {
...
btnSave.addEventListener("click", async () => {
    try {
        let res = await axios.put(`${URL_API}/${idURL}/update-user`, {
