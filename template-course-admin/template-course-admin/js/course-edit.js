...
const show=async()=>{
    try {
        let resId=await axios.get(`http://localhost:8080/api/v1/courses/${param.get('id')}`);
...
courseLogoInput.addEventListener("change",async(e)=>{
    try {
        ...
        let res=await axios.post(`http://localhost:8080/api/v1/courses/${param.get('id')}/files`,formData);
...
btnUpdate.addEventListener("click",async()=>{
    try {
        ...
        let res=await axios.put(`http://localhost:8080/api/v1/courses/${param.get('id')}`,{
...
btnDelete.addEventListener("click",async()=>{
    try {
        await axios.delete(`http://localhost:8080/api/v1/courses/${param.get('id')}`);
    } catch (error) {
        console.log(error);
    }
})
