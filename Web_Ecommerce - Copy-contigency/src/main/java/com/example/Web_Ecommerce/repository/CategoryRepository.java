...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query("SELECT new com.example.Web_Ecommerce.dto.CategoryDto(c.id,c.name,COUNT(p.category.id)) FROM Product p INNER JOIN Category c ON p.category.id=c.id WHERE p.is_activated=true AND p.is_deleted=false GROUP BY c.id")
