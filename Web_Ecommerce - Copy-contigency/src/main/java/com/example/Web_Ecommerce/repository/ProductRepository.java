...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select p from Product p where p.is_activated=true and p.is_deleted=false and p.salePrice<>0")
    List<Product> searchByActivated();

    //Tìm các sản phẩm sắp xếp từ giá cao nhất
    @Query(nativeQuery = true, value = "select * from products p WHERE p.is_activated=true and p.is_deleted=false and p.sale_price!=0 order by p.sale_price DESC limit 6;")
    List<Product> tophighestPriceProduct();

    //Tìm các sản phẩm theo tên và nội dung
    @Query(nativeQuery = true, value = "select * from products p where (p.description like '%w%' or p.name like '%w%') AND p.is_activated=true and p.is_deleted=false and p.sale_price!=0")
    List<Product> searchCustomerProducts();

    //Tìm các sản phẩm theo loại
    @Query(nativeQuery = true, value = "select * from products p where (p.category_id=?1) AND p.is_activated=true and p.is_deleted=false and p.sale_price!=0")
    List<Product> searchCusTomerProductsByCategory(Long id);

    //Tìm các sản phẩm sắp xếp từ thấp đến cao
    @Query(nativeQuery = true, value = "SELECT * \n" +
            " from products p \n" +
            " WHERE p.is_activated=true and p.sale_price!=0 and p.is_deleted=false\n" +
